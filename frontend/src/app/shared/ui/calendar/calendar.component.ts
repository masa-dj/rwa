...
export class CalendarComponent implements OnInit, OnChanges {
    ...
    formatTime(scheduledAt: string): string {
        return new Date(scheduledAt).toLocaleTimeString('en-UK', {
            hour: 'numeric',
            minute: '2-digit',
        });
    }

    studentName(exam: Exam): string {
        if (!exam.student) return '';
        return `${exam.student.firstName} ${exam.student.lastName}`;
    }

    exerciseLabel(exam: Exam): string {
        return this.exerciseLabels[exam.exerciseType];
    }
}
